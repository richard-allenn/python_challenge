...
with open(csvpath) as csvfile:
    ...
    for row in csv_reader:
        if float(row[1]) > float(max_profit):                                       # again
            max_profit = row[1]                                                     
            print(f'the greatest increase in profit is ${max_profit}')
    ...
    print(f'the greatest decrease in profit is ${low_profit}')                       #should be one inside  if but is here to show rest of code works
